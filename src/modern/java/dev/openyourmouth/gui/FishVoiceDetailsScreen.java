...
public final class FishVoiceDetailsScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        int x = width / 2 - 150;
        graphics.text(font, Component.translatable("gui.openyourmouth.fish.catalog.name"), x, top + 2, 0xFFB0B0B0, false);
        graphics.text(font, Component.translatable("gui.openyourmouth.fish.reference_id"), x, top + 34, 0xFFB0B0B0, false);
        if (!status.getString().isBlank()) graphics.centeredText(font, status, width / 2, top + 98, 0xFFFFD54F);
    }
