...
public final class OpenYourMouthClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> dispatcher.register(
                ClientCommands.literal("tts")
                        .executes(context -> { controller.showCommandHelp(); return 1; })
                        .then(ClientCommands.literal("skip").executes(context -> controller.skipCurrent() ? 1 : 0))
                        ...
                        .then(ClientCommands.literal("on").executes(context -> { controller.setEnabled(true); return 1; }))
                        .then(ClientCommands.literal("off").executes(context -> { controller.setEnabled(false); return 1; }))
                        .then(ClientCommands.literal("toggle").executes(context -> { controller.toggleEnabled(); return 1; }))
                        .then(ClientCommands.literal("hearself")
                                .executes(context -> { controller.toggleHearSelf(); return 1; })
                                .then(ClientCommands.literal("on").executes(context -> { controller.setHearSelf(true); return 1; }))
                                .then(ClientCommands.literal("off").executes(context -> { controller.setHearSelf(false); return 1; })))
                        .then(ClientCommands.literal("chat")
                                .executes(context -> { controller.toggleSendChatMessages(); return 1; })
                                .then(ClientCommands.literal("on").executes(context -> { controller.setSendChatMessages(true); return 1; }))
                                .then(ClientCommands.literal("off").executes(context -> { controller.setSendChatMessages(false); return 1; })))
                        .then(ClientCommands.literal("status").executes(context -> { controller.showCommandHelp(); return 1; }))
                        .then(ClientCommands.argument("text", StringArgumentType.greedyString())
                                .executes(context -> controller.speakOnce(StringArgumentType.getString(context, "text")) ? 1 : 0))
        ));
    ...
    public static void openConsentScreen(boolean enableAfterConsent, Runnable onAccepted) {
        ...
        minecraft.execute(() -> minecraft.gui.setScreen(new ConsentScreen(minecraft.gui.screen(), onAccepted, enableAfterConsent)));
    }
}
